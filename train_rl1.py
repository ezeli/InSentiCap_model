    ...
    for epoch in range(opt.rl_epochs):
        ...
        for data_item in tqdm.tqdm(train_data):
            ...
            sample_captions, sample_logprobs, seq_masks = captioner(
                fc_feats, att_feats, cpts_tensor, sample_max=0, max_seq_len=opt.max_seq_len, mode='rl')

            captioner.eval()
            with torch.no_grad():
                greedy_captions, _, _ = captioner(
                fc_feats, att_feats, cpts_tensor, sample_max=1, max_seq_len=opt.max_seq_len, mode='rl')
            captioner.train()
